...
class MainForm(MainWindowUI):
    ...
    def _get_callbacks(self) -> Dict[str, Callable]:
        ...
        return {
            ...
            'load_gml': self._load_gml,
            'load_html': self._load_html,
            ...
            # Tools Menu
            'clean_sequences': self._clean_sequences,
            'sequence_info': self._get_sequence_info,
            'date_to_number': self._date_to_number,
    ...
    def _load_gml(self):
        """Load GML file generated by fastHaN or other software."""
        file_path, _ = QFileDialog.getOpenFileName(
            self,
            "Load GML File",
            "",
        ...
        try:
            self.log_tab.append_info(f"Loading GML: {file_path}")

            # Get output path for processing
            output_path = self.get_output_path()
            if not output_path:
                QMessageBox.warning(self, "Warning", "Please set output directory first!")
                return

            # Generate visualization from GML
            timestamp = int(time.time())

            # Copy GML to output directory
            gml_dest = os.path.join(output_path, f"{timestamp}.gml")
            ...
            self.log_tab.append_error(f"Failed to load GML: {str(e)}")
            QMessageBox.critical(self, "Error", f"Failed to load GML: {str(e)}")

    def _load_html(self):
        """Load HTML file for display in Network Viewer."""
        file_path, _ = QFileDialog.getOpenFileName(
            self,
            "Load HTML File",
            "",
        ...
        try:
            self.log_tab.append_info(f"Loading HTML: {file_path}")

            # Load HTML directly into network viewer
            self.web_view_main.setUrl(QUrl.fromLocalFile(file_path))
            self.switch_to_tab('network')

            self.log_tab.append_success(f"HTML file loaded: {os.path.basename(file_path)}")

        except Exception as e:
            self.log_tab.append_error(f"Failed to load HTML: {str(e)}")
    ...
    def _clean_sequences(self):
        """Clean sequences."""
        self.log_tab.append_info("Clean sequences")
        QMessageBox.information(self, "Info", "Clean sequences - to be implemented")

    def _get_sequence_info(self):
        """Get sequence information."""
        self.log_tab.append_info("Get sequence info")
        QMessageBox.information(self, "Info", "Sequence info - to be implemented")

    def _date_to_number(self):
        """Convert date strings to decimal numbers."""
        delimiter, ok = QInputDialog.getText(
            self, "Date Delimiter", "Enter the date delimiter:", text="-"
        )
        if not ok:
            return
        ...
        for i in range(self.table_model.rowCount()):
            taxon = self.table_model.get_taxon(i)
            if taxon and not self._is_numeric(taxon.continuous_traits):
                parts = taxon.continuous_traits.split(delimiter)
                try:
                    if len(parts) == 2:
                        value = float(parts[0]) + float(parts[1]) / 12.0
                        taxon.continuous_traits = f"{value:.4f}"
                        converted += 1
                    elif len(parts) == 3:
                        value = float(parts[0]) + float(parts[1]) / 12.0 + float(parts[2]) / 30.0 / 12.0
                        taxon.continuous_traits = f"{value:.4f}"
                        converted += 1
                except ValueError:
                    pass
        ...
        self.log_tab.append_info(f"Converted {converted} date values")
